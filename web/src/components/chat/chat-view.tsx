...
function PersonaHeader({ persona }: { persona: TeamMember }) {
  const initials = persona.persona.display_name.charAt(0);
  return (
    <div className="ml-9 mb-1 flex items-center gap-2 text-xs">
      ...
      <span className="text-slate-400">
        · {persona.persona.title || persona.role}
      </span>
    </div>
  );
}
...
export function ChatContent({
  ...
  // Load Virtual Dev Team members for the assignee selector — team mode only
  useEffect(() => {
    if (mode !== "team") return;
  ...
  // Run assignment cache, populated lazily as new run_ids are seen in
  // messages. Used to render persona headers above agent replies.
  const [runAssignments, setRunAssignments] = useState<
    Record<string, { assignee: string | null; team_members: string[] | null }>
  >({});
  ...
  useEffect(() => {
    ...
    for (const m of messages) {
      if (m.run_id && !runAssignments[m.run_id]) seen.add(m.run_id);
    }
    seen.forEach((rid) => {
      apiGet<{ assignee: string | null; team_members: string[] | null }>(
        `/v1/team/runs/${rid}/assignment`
      )
        .then((a) =>
          setRunAssignments((prev) => ({ ...prev, [rid]: a }))
        )
        .catch(() => {
          // Mark as fetched to avoid retry loops
          setRunAssignments((prev) => ({
            ...prev,
            [rid]: { assignee: null, team_members: null },
          }));
        });
    });
  }, [mode, messages, runAssignments]);

  function personaForMessage(
    runId: string | null
  ): TeamMember | null {
    if (mode !== "team" || !runId) return null;
    const a = runAssignments[runId];
    if (!a || !a.assignee) return null;
    return teamMembers.find((m) => m.name === a.assignee) ?? null;
  }
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (rid && runEventsMap[rid] && runEventsMap[rid].length > 0) {
        ...
        if (rid !== currentRun?.run_id) {
          const persona = personaForMessage(rid);
          items.push(
            <div key={`tl:${rid}`}>
              {persona && <PersonaHeader persona={persona} />}
              <AgentThinking events={runEventsMap[rid]} isRunning={false} />
            </div>,
          );
        }
        continue;
